resource "aws_inspector2_enabler" "delegated_admin_account" {
  count          = var.enable_amazon_inspector ? 1 : 0
...
resource "aws_inspector2_member_association" "itgix_primary" {
  count      = var.enable_amazon_inspector ? length(var.organization_member_account_ids) : 0
